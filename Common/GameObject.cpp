...
{
	m_Model.FrustumCulling(frustumInWorld, m_Transform.GetLocalToWorldMatrixXM());
}
...
void GameObject::Draw(ID3D11DeviceContext * deviceContext, IEffect * effect)
{
	...
	for (auto& part : m_Model.modelParts)
	{
		...
		if (pEffectMaterial)
		{
			pEffectMaterial->SetAmbinet(part.ambient);
			pEffectMaterial->SetDiffuse(part.diffuse);
			pEffectMaterial->SetSpecular(part.specular);
		}
...
void GameObject::DrawInstanced(ID3D11DeviceContext* deviceContext, IEffect * effect, const std::vector<Transform> & data)
{
	...
	for (auto& t : data)
	{
		...
		iter->worldInvTranspose = XMath::InverseTranspose(W);
		iter++;
	}
