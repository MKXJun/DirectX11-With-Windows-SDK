...
bool GameApp::InitResource()
{
	...
	// 球体(预先设好包围球)
	m_ModelManager.CreateFromGeometry("Sphere", Geometry::CreateSphere());
	m_Sphere.SetModel(m_ModelManager.GetModel("Sphere"));
	m_BoundingSphere.Center = XMFLOAT3(-5.0f, 0.0f, 0.0f);
	m_BoundingSphere.Radius = 1.0f;
	// 立方体
	m_Cube.SetModel(m_ModelManager.CreateFromGeometry("Cube", Geometry::CreateBox()));
	// 圆柱体
	m_Cylinder.SetModel(m_ModelManager.CreateFromGeometry("Cylinder", Geometry::CreateCylinder()));
	// 房屋
	...
	m_Triangle.SetModel(m_ModelManager.CreateFromGeometry("Triangle", m_TriangleMesh));
