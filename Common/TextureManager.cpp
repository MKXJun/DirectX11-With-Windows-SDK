...
ID3D11ShaderResourceView* TextureManager::CreateTexture(const std::wstring& filename, bool enableMips)
{
	...
	if (FAILED(DirectX::CreateDDSTextureFromFile(m_pDevice.Get(), 
		...
		filename.c_str(), nullptr, res.GetAddressOf())))
	{
		...
		if (img_data)
		{
			CD3D11_TEXTURE2D_DESC texDesc(DXGI_FORMAT_R8G8B8A8_UNORM, width, height, 1, 
				enableMips ? 0 : 1, 
