...
public:
    ...
    void GetCascadePartitions(float output[4]) const { memcpy_s(output, sizeof m_CascadePartitionsFrustum, m_CascadePartitionsFrustum, sizeof m_CascadePartitionsFrustum); }
...
public:
    ...
    // 级联每帧相关的配置
    //
    float		m_CascadePartitionsPercentage[4]{       // 0到100的值表示视锥体所占百分比
        0.04f, 0.10f, 0.25f, 1.0f
    };
