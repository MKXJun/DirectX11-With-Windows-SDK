...
HRESULT OITRender::InitResource(ID3D11Device* device, UINT width, UINT height, UINT multiple)
{
	...
	hr = CreateConstantBuffer(device, initVals, 16, m_pConstantBuffer.ReleaseAndGetAddressOf());
	if (FAILED(hr))
		return hr;
	// 创建结构化缓冲区
	hr = CreateStructuredBuffer(device, nullptr, 12 * width * height * multiple, 12,
		m_pFLBuffer.ReleaseAndGetAddressOf(), false, true);
	if (FAILED(hr))
		return hr;
	// 创建字节地址缓冲区
	hr = CreateRawBuffer(device, nullptr, 4 * width * height,
		m_pStartOffsetBuffer.ReleaseAndGetAddressOf(), false, true);
	if (FAILED(hr))
		return hr;
	...
	hr = device->CreateShaderResourceView(m_pFLBuffer.Get(), &srvDesc,
		m_pFLBufferSRV.ReleaseAndGetAddressOf());
	if (FAILED(hr))
		return hr;
	...
	hr = device->CreateShaderResourceView(m_pStartOffsetBuffer.Get(), &srvDesc,
		m_pStartOffsetBufferSRV.ReleaseAndGetAddressOf());
	if (FAILED(hr))
		return hr;
	...
	hr = device->CreateUnorderedAccessView(m_pFLBuffer.Get(), &uavDesc, m_pFLBufferUAV.ReleaseAndGetAddressOf());
	if (FAILED(hr))
		return hr;
	...
	hr = device->CreateUnorderedAccessView(m_pStartOffsetBuffer.Get(), &uavDesc, m_pStartOffsetBufferUAV.ReleaseAndGetAddressOf());
	if (FAILED(hr))
		return hr;

	// 创建顶点着色器
	ComPtr<ID3DBlob> blob;
	hr = CreateShaderFromFile(L"HLSL\\OIT_Render_VS.cso", L"HLSL\\OIT_Render_VS.hlsl", "VS", "vs_5_0", blob.ReleaseAndGetAddressOf());
	if (FAILED(hr))
		return hr;
	hr = device->CreateVertexShader(blob->GetBufferPointer(), blob->GetBufferSize(), nullptr, m_pOITRenderVS.ReleaseAndGetAddressOf());
	if (FAILED(hr))
		return hr;
	// 创建顶点输入布局
	...
	if (FAILED(hr))
		return hr;
	hr = device->CreatePixelShader(blob->GetBufferPointer(), blob->GetBufferSize(), nullptr, m_pOITRenderPS.ReleaseAndGetAddressOf());
	if (FAILED(hr))
		return hr;
	hr = CreateShaderFromFile(L"HLSL\\OIT_Store_PS.cso", L"HLSL\\OIT_Store_PS.hlsl", "PS", "ps_5_0", blob.ReleaseAndGetAddressOf());
	if (FAILED(hr))
		return hr;
	hr = device->CreatePixelShader(blob->GetBufferPointer(), blob->GetBufferSize(), nullptr, m_pOITStorePS.ReleaseAndGetAddressOf());
