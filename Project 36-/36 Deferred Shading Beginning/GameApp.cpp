...
void GameApp::OnResize()
{
	...
	if (m_pCamera != nullptr)
	{
		...
		m_pForwardEffect->SetProjMatrix(m_pCamera->GetProjMatrixXM());
		m_pDeferredEffect->SetProjMatrix(m_pCamera->GetProjMatrixXM());
...
void GameApp::UpdateScene(float dt)
{
	...
	if (ImGui::Begin("Deferred Shading Beginning"))
	{
	...
	// 注意：由于用了反转Z，在构建视锥体对象时需要把 近/远平面交换回来
...
bool GameApp::InitResource()
{
	...
	// 注意：反转Z时需要将近/远平面对调
	m_pCamera->SetFrustum(XM_PI / 3, AspectRatio(), 300.0f, 0.5f);
	camera->LookAt(XMFLOAT3(60.0f, 10.0f, 5.0f), XMFLOAT3(0.0f, 0.0f, 0.0f), XMFLOAT3(0.0f, 1.0f, 0.0f));
	...
	m_pForwardEffect->SetProjMatrix(camera->GetProjMatrixXM());
	m_pDeferredEffect->SetViewMatrix(camera->GetViewMatrixXM());
	m_pDeferredEffect->SetProjMatrix(camera->GetProjMatrixXM());
	m_pDeferredEffect->SetCameraNearFar(0.5f, 300.0f);
	...
	// 初始化天空盒纹理
	//
	HR(CreateDDSTextureFromFile(m_pd3dDevice.Get(), L"..\\Texture\\Clouds.dds", nullptr, m_pTextureCubeSRV.GetAddressOf()));
...
void GameApp::RenderGBuffer()
{
	...
	if (m_ClearGBuffers)
	{
		for (auto rtv : m_pGBufferRTVs)
			m_pd3dImmediateContext->ClearRenderTargetView(rtv, Colors::Black);
	}
	m_pd3dImmediateContext->ClearDepthStencilView(m_pDepthBuffer->GetDepthStencil(), D3D11_CLEAR_DEPTH | D3D11_CLEAR_STENCIL, 0.0f, 0);
...
void GameApp::RenderSkyboxAndToneMap()
{
	...
	m_pSkyboxEffect->SetProjMatrix(m_pCamera->GetProjMatrixXM());
